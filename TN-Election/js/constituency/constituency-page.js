...
function buildCandidateEntry(cand, allInConst, constName){
  ...
  return {
    ...
    party_full:cand.party_full||party,
    // Preserve live vote data from source
...
// ─────────────────────────────────────────────────────────────────
// LIVE VOTE HELPERS — used only by the Contesting Candidates section
// ─────────────────────────────────────────────────────────────────

/**
 * Returns the numeric live vote count for a candidate, or undefined
 * if no vote data is present on that object.
 */
...
/**
 * Sorts candidates by live votes descending.
 * Candidates with no vote data are pushed to the end.
 */
...
/**
 * Whether any candidate in the list has vote data at all.
 */
...
// rank: 'leading' | 'trailing' | '' (used only in Contesting section)
// votes: number | undefined
// ─────────────────────────────────────────────────────────────────
function buildCandidateCard(cand, rank, votes){
  var pc = getPartyColor(cand.party);
  var ico = getPartyIcon(cand.party);
  var ph = cand.photo
    ? '<img src="'+cand.photo+'" alt="'+cand.name+'" onerror="this.onerror=null;this.src=\'../assets/images/candidates/default/default.png\';" />'
  ...
  // Live votes slot - ALWAYS rendered so all cards stay level.
  var votesHTML =
...
// Changes vs original:
//   1. When live vote data exists → sort by votes, show leading/trailing badges
//   2. When no vote data → fall back to original party-based sort (unchanged behaviour)
//   3. Top 5 shown; rest goes to "View All"
// ─────────────────────────────────────────────────────────────────
function renderCandidates(constId){
  var container = document.getElementById('candidates-scroll');
  if(!container) return;
  ...
  var sortedAll;
  if(liveMode){
    // Primary sort: live votes descending; preserves full ranked order
    sortedAll = sortCandidatesByVotes(allCandidates);
  } else {
    // Fallback: original alliance/party sort — behaviour identical to before
  ...
  container.innerHTML = visible.map(function(cand, idx){
    ...
    return buildCandidateCard(cand, rank, votes);
  }).join('');

  // Click handler for candidate popup (unchanged)
  container.addEventListener('click', function(e){
    var card = e.target.closest('.cand-card');
    if(!card || !card.dataset.candidateId) return;
    var id = card.dataset.candidateId;
...
// ─────────────────────────────────────────────────────────────────
// CSS — injected once at runtime, scoped to candidate cards only
// ─────────────────────────────────────────────────────────────────
(function injectCandidateVoteStyles(){
  ...
  s.textContent = [
    ...
    /* Rank badge — inline pill below the party icon, above candidate name */
...
// ─────────────────────────────────────────────────────────────────
// PIE + CENSUS + ASSEMBLY — all unchanged
// ─────────────────────────────────────────────────────────────────
function drawPie(canvasId, segments){
  var canvas=document.getElementById(canvasId);
  if(!canvas||!canvas.getContext) return;
...
// ─────────────────────────────────────────────────────────────────
// VIEW ALL MODAL
// Now also sorts by votes (when available) and shows rank labels
// ─────────────────────────────────────────────────────────────────
function viewAllCandidates(constId){
  var constMeta=constituenciesData[constId]||{};
  var constKey=normalizeConstituencyKey(constMeta.name);
